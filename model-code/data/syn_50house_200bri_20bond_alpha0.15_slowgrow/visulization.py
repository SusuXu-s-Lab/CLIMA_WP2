...
def visulize_state():
    ...
    df = pd.read_csv("household_states_raw.csv")
...
def visulize_links():
    df = pd.read_csv("ground_truth_network_raw.csv")
...
def visulize_sim():
    similarity_df_loaded = pd.read_csv("similarity_df_raw.csv", index_col=0)
    ...
    plt.close()
def visulize_prob():
    df=pd.read_csv('link_probs_raw.csv')
    # Plot distributions
...
def visualize_p_values_gif():
    ...
    p_self_df = pd.read_csv("p_self_all_values.csv")
    p_ji_df = pd.read_csv("p_ji_all_values.csv")
