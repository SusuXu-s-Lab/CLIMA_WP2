...
class SelfJointMLP(nn.Module):
    ...
    Joint self mechanism:
      input : (B, 2D)  -> [z_i || x_i_prev]
      output: (B, D)   -> per-dimension self-activation probabilities
    """
    def __init__(self, D, hidden=64, depth=2):
        super().__init__()
        self.D = D
        self.net = mlp(in_dim=2*D, out_dim=D, hidden=hidden, depth=depth)

    def forward(self, z_i, x_i_prev):
        # z_i, x_i_prev: (B, D)
...
class EdgeJointMonotone(nn.Module):
    ...
    """
    def __init__(self, D, emb_dim=8, hidden=96, depth=3, phi_monotone=True, include_drop_flag=True):
        super().__init__()
        self.D = D
        ...
        # context net: emb(k) + z_i + x_i + z_j + x_j + [both_d1]
        extra = 1 if include_drop_flag else 0
    ...
    def forward(self, phi, k, z_i, x_i, z_j, x_j, both_d1=None, update_stats=False):
        """
