...
def load_model(model_path, device='cpu'):
    ...
    network_type_nn = NetworkTypeNN(feature_dim, L)
    self_nn = SelfActivationNN(feature_dim, L)
    influence_nn = InfluenceNN(feature_dim, L)
    interaction_nn = InteractionFormationNN(feature_dim)
...
def main():
    ...
    # loader = DataLoader(project_root/'data/syn_data_ruxiao', file_list=data_files)
    loader = DataLoader(project_root/'data/syn_data_v3')
    data = loader.load_data()
    train_data, test_data = loader.train_test_split(data, train_end_time=15)
    
    # Load model
    trainer = load_model(project_root/'saved_models/trained_model_L_3_syn_v3_epoch_150.pth')
