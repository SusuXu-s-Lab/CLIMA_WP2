...
def build_dataset(sim: Dict, T_max: Optional[int] = None) -> List[Dict]:
    """
    Build per-(t,i,d) records with attempts from neighbors active in past m steps.
    ...
    Supports influence window m:
    - m=1 (default): only neighbors newly active at t-1 can attempt
    - m>1: neighbors active in any of the past m steps can attempt
    
    Args:
        sim: simulation dict
        T_max: if specified, only build dataset up to time T_max (for train/test split)
    """
    A = sim.get("A", None)
    At = sim.get("At", None)
    X, Y, z, phi = sim["X"], sim["Y"], sim["z"], sim["phi"]
    T, N, D = X.shape
    T_use = T if T_max is None else min(T_max, T)
    drop_dim = sim.get("true_params", {}).get("drop_dim", -1)
    m = sim.get("true_params", {}).get("m", 1)  # default to classic IC (m=1)
    m = max(1, int(m))

    data = []
    for t in range(1, T_use):
        # Compute influence window: nodes active in [t-m, t-1]
        window_start = max(0, t - m)
        if m == 1:
            # Fast path: classic IC (only newly active at t-1)
        ...
        for i in range(N):
            ...
            for d in range(D):
                ...
                attempts = []
                for j in js:
                    if not recently_active[j].any():
                        continue
                    x_j_prev = X[t-1, j].astype(np.float32)
                    z_j = z[j].astype(np.float32)
                    # per-dimension attempts from j that was active in past m steps
                    for k in range(D):
...
def e_step(data, self_net, edge_net, tau_resp=1.0, edge_resp_floor=0.0, device="cpu", use_time=False, T_total=None):
    """
    Returns:
      self_rows: (z_i, x_i, t, d, target) if use_time else (z_i, x_i, d, target)
      edge_rows: (phi, k, z_i, x_i, z_j, x_j, both_d1, t, d, target) if use_time else (phi, k, z_i, x_i, z_j, x_j, both_d1, d, target)
    """
    self_rows = []
    edge_rows = []
    for rec in data:
        y = rec["y"]; d = rec["d"]; t = rec["t"]
        ...
        # Prepare time feature if needed
        if use_time:
            if T_total is None:
                raise ValueError("T_total must be provided when use_time=True")
            t_norm = torch.tensor([[t / (T_total - 1)]], dtype=torch.float32, device=device)
            s_vec = self_net(z_i, x_i, t_norm).squeeze(0)  # (D,)
        else:
        ...
        if rec["attempts"]:
            ...
            if use_time:
                t_norm_rep = t_norm.repeat(len(rec["attempts"]), 1)
                q_mat = edge_net(phi, k, z_i_rep, x_i_rep, z_j, x_j, both, t_norm_rep, update_stats=False)  # (M,D)
            else:
        ...
        if y == 0:
            if use_time:
                self_rows.append((rec["z_i"], rec["x_i"], t, d, 0.0))
                for idx, a in enumerate(rec["attempts"]):
                    edge_rows.append((a["phi"], a["k"], rec["z_i"], rec["x_i"], a["z_j"], a["x_j"], a["both_d1"], t, d, 0.0))
            else:
        ...
        if use_time:
            self_rows.append((rec["z_i"], rec["x_i"], t, d, float(r_self)))
            for (a, r_m) in zip(rec["attempts"], r_edges):
                edge_rows.append((a["phi"], a["k"], rec["z_i"], rec["x_i"], a["z_j"], a["x_j"], a["both_d1"], t, d, float(r_m)))
        else:
...
def _compute_dim_weights_self(self_rows, D, mode="none", device="cpu", use_time=False):
    if mode == "none":
    ...
    if use_time:
        for _, _, _, d, y in self_rows:  # (z_i, x_i, t, d, target)
            sums[d] += float(y)
            cnts[d] += 1.0
    else:
        for _, _, d, y in self_rows:  # (z_i, x_i, d, target)
            sums[d] += float(y)
            cnts[d] += 1.0
...
def _self_loss(rows, self_net, device, dim_weights=None, use_time=False, T_total=None):
    ...
    if use_time:
        ...
        t = torch.tensor([[r[2] / (T_total - 1)] for r in rows], dtype=torch.float32, device=device)  # (B,1)
        d = torch.tensor([r[3] for r in rows], dtype=torch.long, device=device)     # (B,)
        y = torch.tensor([r[4] for r in rows], dtype=torch.float32, device=device)  # (B,)
        s_mat = self_net(z, x, t)                                                   # (B,D)
    else:
        z = torch.tensor([r[0] for r in rows], dtype=torch.float32, device=device)  # (B,D)
        x = torch.tensor([r[1] for r in rows], dtype=torch.float32, device=device)  # (B,D)
...
def _edge_loss(rows, edge_net, device, use_time=False, T_total=None):
    if not rows: return torch.tensor(0.0, device=device)
    ...
    if use_time:
        ...
        t = torch.tensor([[r[7] / (T_total - 1)] for r in rows], dtype=torch.float32, device=device)  # (B,1)
        d   = torch.tensor([r[8] for r in rows], dtype=torch.long, device=device)
        y   = torch.tensor([r[9] for r in rows], dtype=torch.float32, device=device)
        q_mat = edge_net(phi, k, z_i, x_i, z_j, x_j, both, t, update_stats=False)        # (B,D)
    else:
        phi = torch.tensor([r[0] for r in rows], dtype=torch.float32, device=device)
        k   = torch.tensor([r[1] for r in rows], dtype=torch.long, device=device)
        z_i = torch.tensor([r[2] for r in rows], dtype=torch.float32, device=device)
...
def _edge_pairwise_rank_loss(rows, edge_net, device, num_pairs=512, margin=0.02, use_time=False, T_total=None):
    ...
    for idx, r in enumerate(rows):
        if use_time:
            phi, k, _, _, _, _, _, _, d, _ = r  # (phi, k, z_i, x_i, z_j, x_j, both_d1, t, d, target)
        else:
            phi, k, _, _, _, _, _, d, _ = r  # (phi, k, z_i, x_i, z_j, x_j, both_d1, d, target)
    ...
    if use_time:
        ...
        t_lo = torch.tensor([[rows[i][7] / (T_total - 1)] for i in idx_lo.tolist()], dtype=torch.float32, device=device)
        t_hi = torch.tensor([[rows[i][7] / (T_total - 1)] for i in idx_hi.tolist()], dtype=torch.float32, device=device)

        q_lo_mat = edge_net(phi_lo, k, z_i_lo, x_i_lo, z_j_lo, x_j_lo, both_lo, t_lo, update_stats=False)  # (B,D)
        q_hi_mat = edge_net(phi_hi, k, z_i_hi, x_i_hi, z_j_hi, x_j_hi, both_hi, t_hi, update_stats=False)  # (B,D)
    else:
        phi_lo, phi_hi = gather(0)
        z_i_lo, z_i_hi = gather(2)
        x_i_lo, x_i_hi = gather(3)
...
def train_em(sim: Dict,
             ...
             # influence window override
             m_train: Optional[int]=None,  # if provided, override sim's m for training
             # train/test split
             T_max: Optional[int]=None,  # if provided, only train on data up to T_max
    ...
    # Override m if m_train is provided (for mismatch experiments)
    if m_train is not None:
        print(f"[train_em] Overriding data m={sim.get('true_params', {}).get('m', 1)} with m_train={m_train}")
        # Create a modified sim dict with overridden m
        sim = sim.copy()
        if 'true_params' not in sim:
            sim['true_params'] = {}
        sim['true_params'] = sim['true_params'].copy()
        sim['true_params']['m'] = m_train
        sim['m_train'] = m_train  # store for evaluation
    
    data = build_dataset(sim, T_max=T_max)
    T_total = sim["X"].shape[0]  # Total number of time steps

    self_net = SelfJointMLP(D, hidden=self_hidden, depth=self_depth, use_time=use_time).to(device)
    edge_net = EdgeJointMonotone(D, emb_dim=edge_emb, hidden=edge_hidden, depth=edge_depth,
                                 phi_monotone=phi_monotone, include_drop_flag=True, use_time=use_time).to(device)
    ...
    for it in range(em_iters):
        ...
        if phi_warmup_iters > 0 and it < phi_warmup_iters:
            ...
            tmp_self_rows, tmp_edge_rows = e_step(
                ...
                use_time=use_time, T_total=T_total
            )
            if tmp_edge_rows:
        ...
        # Diagnostic: print responsibility statistics
        if use_time:
            r_self_vals = np.array([r[4] for r in self_rows])  # (z_i, x_i, t, d, target)
            r_edge_vals = np.array([r[9] for r in edge_rows]) if edge_rows else np.array([])  # (..., t, d, target)
        else:
            r_self_vals = np.array([r[3] for r in self_rows])  # (z_i, x_i, d, target)
            r_edge_vals = np.array([r[8] for r in edge_rows]) if edge_rows else np.array([])  # (..., d, target)
        if r_edge_vals.size > 0:
            print(f"[EM {it+1:02d}] r_self.mean={r_self_vals.mean():.4f}, "
                  f"r_edge.mean={r_edge_vals.mean():.4f}, "
                  f"frac_edge>0.01={np.sum(r_edge_vals > 0.01) / r_edge_vals.size:.4f}")

        # per-state weights for self
        dim_weights = _compute_dim_weights_self(self_rows, D, mode=self_weighting, device=device, use_time=use_time)

        # M-step: self
        for _ in range(epochs_self):
            opt_s.zero_grad()
            loss_s = _self_loss(self_rows, self_net, device, dim_weights=dim_weights, use_time=use_time, T_total=T_total)
            loss_s.backward()
        ...
        for _ in range(epochs_edge):
            opt_e.zero_grad()
            loss_e = _edge_loss(edge_rows, edge_net, device, use_time=use_time, T_total=T_total)
